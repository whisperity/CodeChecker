...
''')
        ...
        log_argument_help_msg = "Path to the log file which is created "     \
                                "during the build. \nIf there is an already "\
                                "generated log file with the compilation "   \
                                "commands\ngenerated by 'CodeChecker log' "  \
                                "or 'cmake -DCMAKE_EXPORT_COMPILE_COMMANDS' "\
                                "\n CodeChecker check can use it for the "   \
                                "analysis in that case running the original "\
                                "build will \nbe left out from the analysis "\
                                "process (no log is needed)."

        suppress_help_msg = "Path to suppress file.\nSuppress file can be "  \
                            "used to suppress analysis results during the "  \
                            "analysis.\nIt is based on the bug identifier "  \
                            "generated by the compiler which is "            \
                            "experimental.\nDo  not depend too much on this "\
                            "file because identifier or file format can be " \
                            "changed.\nFor other in source suppress features"\
                            " see the user guide."
...
Build command which is used to build the project.''')
        ...
        # --------------------------------------
        # Checker daemon. ("CodeChecker-as-a-Service")
        daemon_parser = subparsers.add_parser('daemon',
                                              formatter_class=ADHF,
                                              help='Start the CodeChecker '
                                                   'remote checker server.')
        daemon_parser.add_argument('-w', '--workspace', type=str,
                                   dest="workspace",
                                   default=util.get_default_workspace(),
                                   help=workspace_help_msg)

        daemon_parser.add_argument('--port', type=int, dest="port",
                                   default=8002, required=False,
                                   help='Port used for daemon connection.')

        daemon_parser.add_argument('--host', type=str,
